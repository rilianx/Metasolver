...
int main(int argc, char** argv){
	...
	args::ValueFlag<double> _diversity_threshold(parser, "double", "Diversity Threshold", {"div"});
	...
	double alpha=4.0, beta=1.0, gamma=0.2, delta=1.0, p=0.04, maxtheta=0.0, diversity_threshold=0.0;
	...
	if(_p) p=_p.Get();
	if(_diversity_threshold) diversity_threshold=_diversity_threshold.Get();
...
//a las cajas se les inicializan sus pesos en 1
	...
	cout << "bsg" << endl;
    BSG *bsg= new BSG(vcs,*gr, 4, 0, 0, diversity_threshold);
